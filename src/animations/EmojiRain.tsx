export class EmojiRain {
  ...
  public initEmojis() {
    ...
    for (var i = 0; i < 15; i++) {
      this.addEmoji(i * 150, [10 + 0, 300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 + 0, -300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 - 200, -300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 + 200, 300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 - 400, -300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 + 400, 300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 - 600, -300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
      this.addEmoji(i * 150, [10 + 600, 300], this.emojiTypes[Math.floor(Math.random() * this.emojiTypes.length)]);
    }
  }
  ...
  private addEmoji(delay: number, range: number[], color: string) {
    setTimeout(() => {
      var e = new Emoji(range[0] + Math.random() * range[1], Math.random() * 4, color, {
  ...
  private spawnNewEmoji() {
    ...
    const ranges = [
      [10 + 0, 300], [10 + 0, -300], [10 - 200, -300], [10 + 200, 300],
      [10 - 400, -300], [10 + 400, 300], [10 - 600, -300], [10 + 600, 300]
    ];
  ...
  public rainEmojis() {
    ...
    this.spawnInterval++;
    if (this.spawnInterval >= 30) { // Spawn every 30 frames (roughly every 0.5 seconds at 60fps)
      this.spawnNewEmoji();
      this.spawnInterval = 0;
    }
...
class Emoji {
  ...
  constructor(x: number, y: number, c: string, v: { x: number, y: number }, range: number[], screenHeight: number) {
    this.container = document.getElementById('animate') as HTMLElement;
    this.x = x;
    this.y = y;
    this.color = c;
